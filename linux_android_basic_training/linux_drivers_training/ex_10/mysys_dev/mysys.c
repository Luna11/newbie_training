...
*
* Description: 
*	This file base on ex05 concurr.c and only and 
*	value file under /sys/class/scull0/scull0.
*	There are not show and store function and I will 
*	complete them later.
*
* Author/Created Date: 
*	Luna Wang, Otc30'14
...
//static ssize_t val_show(struct scull_chardev* dev, struct )

static DEVICE_ATTR(value, 0700, NULL, NULL);         /*********DEVICE_ATTR***********/
...
//	sema_init(&(scull_cdevp->sem), 1);
	...
	cls=class_create(THIS_MODULE, "scull0");     /*******在/sys/class/目录下创建设备类别目录scull0******/
	temp=device_create(cls, NULL, devno, NULL, "scull0");  /****在/dev/目录和/sys/class/scull0目录下\
	分别创建设备文件scull0*********/

	device_create_file(temp, &dev_attr_value);  /****在/sys/class/scull0/scull0目录下创建属性文件value****/
//	dev_set_drvdata(temp, scull_cdevp);
//sema_init(&(scull_cdevp->sem), 1);
